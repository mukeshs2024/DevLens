...
from ai.services.groq_client import AIModelUnavailableError, GroqClient
...
async def analyze(request: AnalysisRequest) -> AnalysisResponse | JSONResponse:
    try:
        ...
        return AnalysisResponse(
            summary="Fallback: Groq API quota exceeded or unavailable.",
            root_cause="Fallback: The DevLens AI encountered an availability limit while communicating with the Groq API.",
            severity="High",
            suggested_fix="Fallback: Check your Groq Cloud Project billing, upgrade your quota, or wait for capacity to return.",
            confidence=0.9,
        )
